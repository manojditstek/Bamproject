...
=======
  ...
  setup() {
    ...
    const {
      ...
      key: cardConfig.value.payment_intent
        ? cardConfig.value.payment_intent.publishable_key
        : "pk_test_guTC6Gf1mA5drZHtmEGImgC600HIXNXoTd" || "",
      elements: [
        {
          type: payMethod.value,
          options: {}
>>>>>>> Stashed changes
    ...
    const registerCard = async () => {
      if (event.value?.complete) {
        ...
        console.log(
          "cardConfig12secret==>",
          cardConfig.value.payment_intent.client_secret
        );
    ...
    const registerBank = async () => {
      if (event.value?.complete) {
        ...
        const response = await stripeEps.value?.confirmIdealPayment(
          cardConfig.value.payment_intent.client_secret,
          {
            payment_method: {
